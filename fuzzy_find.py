...
with open("fuzzy_report.txt", "w") as f:
    if not duplicates:
        ...
        for r1, r2, reason in duplicates:
            ...
            f.write(f"  A [{r1['run_id']}]: {r1['start_date_local']} | {r1.get('distance')}m | HR: {r1.get('average_heartrate')}\n")
            f.write(f"  B [{r2['run_id']}]: {r2['start_date_local']} | {r2.get('distance')}m | HR: {r2.get('average_heartrate')}\n")
            f.write("\n")
