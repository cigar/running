...
for r1, r2 in duplicates:
    ...
    # 1. Heart rate
    if r1.get("average_heartrate"):
        r1_score += 100
    if r2.get("average_heartrate"):
        r2_score += 100

    # 2. Location
    if r1.get("location_country"):
        r1_score += 10
    if r2.get("location_country"):
        r2_score += 10

    # 3. Distance
    if r1.get("distance", 0) > r2.get("distance", 0):
        r1_score += 1
    if r2.get("distance", 0) > r1.get("distance", 0):
        r2_score += 1
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM activities WHERE run_id = ?", (run_to_delete["run_id"],)
        )
